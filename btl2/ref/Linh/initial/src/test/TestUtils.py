...
class TestAST:
    ...
    def checkASTGen(input,expect,num):
        ...
        #print("solution:   " + line)
        #print("expect:     " + expect)
        f = open("./test/AntiASTGenSuite.py", "a+")
        f.write("\tdef test_"+ str(num %100)+  "(self):\n\t\t\"\"\"AST_TEST_" + str(num%100)+ "\"\"\"\n")
        f.write("\t\tinput="+"\"\"\""+input+"\"\"\"\n")
        f.write("\t\t"+ line+"\n")
        f.write("\t\tself.assertTrue(TestAST.checkASTGen(input,expect," + str(num)+  "))\n" )
        f.close()
        return line == expect
